...
## 第 7 步：配置降价提醒推送（可选，但强烈建议）

> 设置页的「异动推送」依赖微信「订阅消息」。不配置也能正常用小程序，只是打开推送开关时会提示模板未配置。
> 推送规则：当银行卖出价相对**今日最高价**下降达到设定金额（如 10 元/克）时，自动推送机构名、当前价、今日最高价。
...
3. 选一个有这些字段的模板：
   ...
   - 机构名称（thing 类型）
   - 时间（time 类型，可选）
4. 添加后得到模板 ID（形如 xXxXxXxX...）→ 复制下来
```

> 如果找不到完全匹配的模板，就选字段名最接近的；后面按模板字段名改一下云函数里的 `data` 字段即可。
...
打开 `cloudfunctions/fetchGoldNow/index.js`，找到 `checkAndNotify` 函数里的 `cloud.openapi.subscribeMessage.send`。`data` 字段名必须和你申请的模板字段一一对应。例如：

```js
data: {
  thing1: { value: '工商银行降价提醒' },   // 标题 / 提醒事项
  amount2: { value: '760.50元/克' },       // 当前价格
  amount3: { value: '770.50元/克' },       // 今日最高价格
  thing4: { value: '工商银行' },            // 机构名称
  time5: { value: '2025-06-18 15:30' }    // 时间（如果模板有时间字段）
}
```

如果模板里没有 `time5` 字段，就把这一行删掉；如果字段名是 `thing1/amount2/amount3/thing4`，则一一对应。模板后台会显示每个字段的英文名，照着填即可。

> ⚠️ 微信订阅消息是「一次性」的：用户每授权一次只能收 1 条提醒。设置页提供「续订」按钮，配额用尽后需重新点击授权。推送成功后会自动扣减 1 条配额。
...
| 7 | 切换到"5 分钟" | 显示"保存成功" |
| 8 | 打开"降价提醒"开关 | 弹出微信订阅授权，点"允许"后显示"订阅成功" |
| 9 | 调整"降价提醒阈值"为 10 元 | 选项高亮切换，自动同步到云端 |
