 ...
 * 交通银行积存金 —— 东方财富代理
 *
 * 交行没有公开 API，使用东财 Au99.99 报价 + 经验价差
 * 价差：卖出价 = 东财价 + 1.8 元/克
 */
...
class BCMSource extends BaseSource {
  ...
  async fetchOne() {
    ...
    if (!data || !data.data) throw new Error('bcm: eastmoney fetch failed');
    const d = data.data;
    const eastPrice = d.f43 / 100;
    const lastClose = d.f60 / 100;
    const change = d.f169 / 100;

    if (!eastPrice || eastPrice < 100) throw new Error('bcm: invalid eastmoney price');
